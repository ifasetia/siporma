...
<div id="eventEditModal" class="fixed inset-0 z-999999 hidden p-5 overflow-y-auto">
    ...
    <div class="min-h-full flex items-center justify-center">
        <div class="modal-dialog relative w-full max-w-[700px] rounded-3xl bg-white p-6 lg:p-11">
            ...
            <form id="submitFormEditProject" class="flex flex-col gap-5" method="POST" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <input name="title" class="input-field h-11 w-full rounded-lg border border-gray-300 px-4 text-sm">
                ...
                <div>
                    ...
                    <button type="button" id="btnAddEditLink" class="mt-2 text-sm text-blue-600 hover:underline">
                        + Tambah Link
                    </button>
                </div>
        ...
        `);
                ...
                $(document).on('click', '.btn-delete-photo', function () {
                    ...
                    $.ajax({
                        ...
                        type: 'POST', // ubah dari DELETE
                        data: {
                            _token: $('meta[name="csrf-token"]').attr('content'),
                            _method: 'DELETE' // ini yang penting
                        },
                        success: function () {
                ...
                `);
                    ...
                    if (p.files && p.files.length) {
                        p.files.forEach(f => {
                            ...
                            $('#oldFileList').append(`
                    <li class="flex justify-between items-center old-file-${f.id}">
                        <span>${filename}</span>
                        <button type="button"
                    ...
                    if (p.photos && p.photos.length) {
                        ...
                        p.photos.forEach(ph => {
                            ...
                            $('#oldPhotoPreview').append(`
                                <div class="relative photo-item" data-id="${ph.id}">
                                    <img src="/storage/${ph.photo}"
                                        class="h-20 rounded-lg object-cover">

                                    <button type="button"
                                        data-id="${ph.id}"
                                        class="btn-delete-photo absolute top-0 right-0 bg-red-500 text-white w-5 h-5 text-xs rounded-full">
                                        ✕
                                    </button>
                                </div>
                            `);
