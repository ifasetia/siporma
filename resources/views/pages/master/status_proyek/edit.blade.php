<div id="eventEditModal" class="fixed inset-0 z-999999 hidden p-5 overflow-y-auto" role="dialog" aria-modal="true">
    ...
    <div class="min-h-full flex items-center justify-center">
        <div class="modal-dialog relative w-full max-w-[700px] rounded-3xl bg-white p-6 lg:p-11 ">
            ...
            <form id="submitFormEditStatusProyek" class="flex flex-col gap-6" action="" method="POST"
                enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div>
                    <h5 id="eventEditModalLabel" class="font-semibold text-gray-800 text-xl dark:text-white">
                        Edit Data Status Proyek
                    </h5>
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        Silahkan isi form berikut untuk mengedit master data status proyek.
                    </p>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-4">
                    ...
                    <button type="button" id="btnEditStatusProyek"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function resetStatusProyekEditForm() {
            const form = $('#submitFormEditStatusProyek');
            form[0].reset();
            $('.error').text('');
        }
        ...
        /*
        |--------------------------------------------------------------------------
        | ✏️ EDIT DATA STATUS PROYEK - LOAD DATA + FULL SWEETALERT LOADING
        |--------------------------------------------------------------------------
        */
        ...
        function openEditModal(data) {
            ...
            const form = $('#submitFormEditStatusProyek');
            form.attr('action', `/status-proyek/${data.sp_id}/update`);

            // Fill form fields with existing data
            form.find('input[name="sp_nama_status"]').val(data.sp_nama_status);
            form.find('input[name="sp_keterangan"]').val(data.sp_keterangan);
            form.find('select[name="sp_warna"]').val(data.sp_warna);
        }

        /*
        |--------------------------------------------------------------------------
        | ✏️ UPDATE DATA STATUS PROYEK
        |--------------------------------------------------------------------------
        */
        $('#btnEditStatusProyek').on('click', function (e) {
            e.preventDefault();
            const form = $('#submitFormEditStatusProyek');
...
@endpush<div id="eventEditModal" class="fixed inset-0 z-999999 hidden p-5 overflow-y-auto" role="dialog" aria-modal="true">
