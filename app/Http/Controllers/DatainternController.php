    ...
    public function datatable(Request $request)
{
    ...
    return DataTables::of($query)
        ...
        ->addColumn('pr_kampus', fn ($row) => $row->profile->pr_kampus ?? '-')
        ...
        ->addColumn('detail', function ($row) {
            return '
                <button
                    type="button"
                    data-id="'.$row->id.'"
                    class="btn-detail inline-flex items-center gap-1 rounded-md bg-indigo-50 px-2 py-1 text-xs text-indigo-600">
                    Detail
                </button>';
        })
            ...
            return '
                <div class="flex items-center justify-center gap-1.5">
                    ...
                    <button
                        type="button"
                        data-id="'.$row->id.'"
                        class="btn-edit inline-flex items-center gap-1.5 rounded-md bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-600">
                        Edit
                    </button>
                    ...
                    <button
                        type="button"
                        data-id="'.$row->id.'"
                        class="btn-delete inline-flex items-center gap-1.5 rounded-md bg-red-50 px-2.5 py-1 text-xs font-medium text-red-600">
                        Hapus
                    </button>
    ...
    {
        $intern = Profile::with('user')->findOrFail($id);
